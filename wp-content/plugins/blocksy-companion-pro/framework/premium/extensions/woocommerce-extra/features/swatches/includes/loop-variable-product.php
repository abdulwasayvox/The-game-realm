	...
	public function __construct() {
		...
		add_filter(
			...
			function ($images) {
				...
				if ($maybe_current_variation) {
					return blocksy_product_get_gallery_images(
		...
		add_filter(
			...
			function ($price) {
				...
				$variations = blc_get_ext('woocommerce-extra')
					->utils
					->get_available_variations($product->get_id());

				foreach (array_values($variations) as $variation) {
					if (
						$variation['variation_id'] != $maybe_current_variation
					) {
						continue;
					}
